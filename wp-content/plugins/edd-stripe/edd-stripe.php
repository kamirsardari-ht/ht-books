...
define( 'EDD_STRIPE_VERSION', '2.2.1' );
...
function edds_process_stripe_payment( $purchase_data ) {
	...
	if ( ! isset( $_POST['edd_stripe_token'] ) ) {
		...
		if ( isset( $edd_options['stripe_js_fallback'] ) ) {
			...
			$card_data = edds_process_post_data( $purchase_data );
			...
			if ( edds_is_recurring_purchase( $purchase_data ) && ( ! empty( $customer ) || $customer_exists ) ) {
				...
				**********************************************************/
				...
				try {
					...
					$plan_data = apply_filters( 'edd_recurring_stripe_subscription_details', $plan_data, $payment, $user->ID );
...
function edds_process_profile_update( $user_id, $verified ) {
	...
	if ( ! isset( $_POST['edd_stripe_token'] ) ) {
		...
		if ( isset( $edd_options['stripe_js_fallback'] ) ) {
			...
			$card_data = edds_process_post_data();
...
function edds_payment_status_filters( $views ) {
	...
	$views['preapproval'] = sprintf( '<a href="%s"%s>%s</a>', esc_url( add_query_arg( 'status', 'preapproval', admin_url( 'edit.php?post_type=download&page=edd-payment-history' ) ) ), $current === 'preapproval' ? ' class="current"' : '', __( 'Preapproval Pending', 'edd' ) . $preapproval_count );
	$views['cancelled']   = sprintf( '<a href="%s"%s>%s</a>', esc_url( add_query_arg( 'status', 'cancelled', admin_url( 'edit.php?post_type=download&page=edd-payment-history' ) ) ), $current === 'cancelled' ? ' class="current"' : '', __( 'Cancelled', 'edd' ) . $cancelled_count );
